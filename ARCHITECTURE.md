...
To prevent catastrophic forgetting and hallucination while managing four competing objectives (Energy, Carbon, Comfort, Equipment Health), we use a **Unified Structured Prompting** strategy. 

Rather than deploying four separate agent prompts (which multiplies latency and token costs), the prompt forces the LLM to write distinct reasoning chains for each objective *before* emitting the final actuator tool calls. This Chain-of-Thought (CoT) alignment ensures the model physically cannot lower a setpoint without first generating text evaluating the carbon and equipment health impact of that decision.
...
1. **JSON Lines (JSONL) Streaming**: The system supports chunked streaming from the LLM. Instead of waiting for the full response, a custom regex parser aggregates the stream and extracts the JSON tool call block in real-time.
...
- **Rolling Context Builder**: Raw telemetry (e.g., fan power watts, room temperatures) is fed into a `ContextBuilder` which mathematically reduces the data into a 10-step rolling average summary. 
